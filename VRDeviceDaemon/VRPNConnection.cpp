...
VRPNConnection::VRPNConnection(const char* serverName,int serverPort)
	:udpSocket(-1,-1),udpSocketConnected(false),
	 messageBufferSize(32),messageBuffer(new char[messageBufferSize]),
	 senders(17),
...
void VRPNConnection::readNextMessages(void)
	...
	if(udpSocketConnected&&readFds.isSet(udpSocket.getFd()))
		...
		char packet[1500];
		size_t packetSize=udpSocket.receiveMessage(packet,sizeof(packet));
		...
		{
		/* Read the next message header from the TCP socket: */
		char header[4*sizeof(u_int32_t)+sizeof(int32_t)+alignment];
		tcpSocket.blockingRead(header,pad(4*sizeof(u_int32_t)+sizeof(int32_t)));
		...
		/* Determine the length of the message payload and the padded length: */
		...
		if(messageBufferSize<messageLen)
			{
			delete[] messageBuffer;
			messageBufferSize=messageLen;
			messageBuffer=new char[messageBufferSize];
			}
		
		/* Read the message payload: */
		tcpSocket.blockingRead(messageBuffer,messageLen);
		
		/* Handle the message: */
		handleMessage(messageTime,messageType,sender,messageSize,messageBuffer);
		}
	}
