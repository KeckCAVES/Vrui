...
#ifndef THREADS_ASCIIFILEREADER_INCLUDED
#define THREADS_ASCIIFILEREADER_INCLUDED
...
class ASCIIFileReader
	...
	private:
	Thread readAheadThread; // Background thread to read ahead in the input file
	MutexCond readRequestCond; // Condition variable to synchronize main and readahead thread
	unsigned int readRequest; // Index of the current read request
	unsigned int readComplete; // Index of the most recent completed read request
	bool readError; // Flag if any errors occured during the last read
	...
	void* readAheadThreadMethod(void); // Method to read ahead in the input file using a background thread
	...
	const char* readNextToken(void); // Reads the next token, i.e., either a single punctuation character, or a sequence of non-whitespace and non-punctuation characters
