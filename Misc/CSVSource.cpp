...
CSVSource::FormatError::FormatError(unsigned int fieldIndex,size_t recordIndex)
	:std::runtime_error(Misc::printStdErrMsg("CSVSource::read: Format error in field %u of record %u",fieldIndex,(unsigned int)recordIndex))
	{
	}
...
CSVSource::ConversionError::ConversionError(unsigned int fieldIndex,size_t recordIndex,const char* dataTypeName)
	:std::runtime_error(Misc::printStdErrMsg("CSVSource::read: Could not convert field %u of record %u to type %s",fieldIndex,(unsigned int)recordIndex,dataTypeName))
	{
	}
...
CSVSource::CSVSource(CharacterSource& sSource)
	:source(sSource),
