...
tables in XBase (dBASE III, FoxPro, ...) format.
...
XBaseTable::XBaseTable(const char* fileName)
	:file(fileName,"rb",Misc::LargeFile::LittleEndian)
	{
	/* Read the table header: */
	version=file.read<unsigned char>();
	char dummy[16];
	file.read(dummy,3); // Date of last update
	numRecords=file.read<unsigned int>();
	headerSize=file.read<unsigned short>();
	recordSize=file.read<unsigned short>();
	file.read(dummy,2+1); // Reserved; incomplete transaction flag
	if(file.read<char>()!=0x0)
		Misc::throwStdErr("XBaseTable::XBaseTable: Table file %s is encrypted",fileName);
	file.read(dummy,4+8+1+1+2); // Free record thread; reserved; MDX flag; language driver; reserved
	...
	while(true)
		...
		file.read(dummy,4); // Field data address
		field.recordOffset=fieldRecordOffset;
		field.size=file.read<unsigned char>();
		field.numDecimals=file.read<unsigned char>();
		file.read(dummy,2+1+2+1+7+1); // Reserved; work area ID; reserved; SET FIELDS flag; reserved; index field flag
		
		/* Check the field for consistency: */
		if(field.dataType=='N'&&field.numDecimals>field.size)
			Misc::throwStdErr("XBaseTable::XBaseTable: Field \"%s\" in table file %s has size %u, but %u decimals",field.name,fileName,(unsigned int)field.size,(unsigned int)field.numDecimals);
		if(field.dataType!='N'&&field.numDecimals>0)
			Misc::throwStdErr("XBaseTable::XBaseTable: Field \"%s\" in table file %s is not of type NUMBER, but has %u decimals",field.name,fileName,(unsigned int)field.numDecimals);
		if(field.dataType=='L'&&field.size!=1)
			Misc::throwStdErr("XBaseTable::XBaseTable: Field \"%s\" in table file %s is of type BOOLEAN, but has wrong size %u",field.name,fileName,(unsigned int)field.size);
		if(field.dataType=='D'&&field.size!=8)
			Misc::throwStdErr("XBaseTable::XBaseTable: Field \"%s\" in table file %s is of type DATE, but has wrong size %u",field.name,fileName,(unsigned int)field.size);
	...
	if(fields.empty())
		Misc::throwStdErr("XBaseTable::XBaseTable: Table file %s does not define fields",fileName);
	if(recordSize!=size_t(fieldRecordOffset))
		Misc::throwStdErr("XBaseTable::XBaseTable: Table file %s reports record size %u, but has record size %u",fileName,(unsigned int)recordSize,(unsigned int)fieldRecordOffset);
	if(file.tell()>headerSize)
		Misc::throwStdErr("XBaseTable::XBaseTable: Table file %s has %u bytes of header data after field definitions",fileName,(unsigned int)(file.tell()-headerSize));
	if(file.tell()<headerSize)
		Misc::throwStdErr("XBaseTable::XBaseTable: Table file %s has %u bytes of missing header data",fileName,(unsigned int)(headerSize-file.tell()));
	}
...
XBaseTable::Record XBaseTable::readRecord(size_t recordIndex)
	...
	file.seekSet(headerSize+Misc::LargeFile::Offset(recordIndex)*Misc::LargeFile::Offset(recordSize));
	...
	switch(f.dataType)
		...
		case 'L':
			if(s[0]=='Y'||s[0]=='y'||s[0]=='T'||s[0]=='t')
				...
				Misc::throwStdErr("XBaseTable::getFieldBoolean: Unrecognized value \"%c\" in boolean field",s[0]);
			break;
		
		default:
			Misc::throwStdErr("XBaseTable::getFieldBoolean: Mismatching field data type \"%c\"",f.dataType);
		}
...
XBaseTable::Maybe<std::string> XBaseTable::getFieldString(const XBaseTable::Record& record,size_t fieldIndex) const
	...
	switch(f.dataType)
		...
		default:
			Misc::throwStdErr("XBaseTable::getFieldString: Mismatching field data type \"%c\"",f.dataType);
		}
...
XBaseTable::Maybe<int> XBaseTable::getFieldInteger(const XBaseTable::Record& record,size_t fieldIndex) const
	...
	switch(f.dataType)
		...
		default:
			Misc::throwStdErr("XBaseTable::getFieldInteger: Mismatching field data type \"%c\"",f.dataType);
		}
...
XBaseTable::Maybe<double> XBaseTable::getFieldNumber(const XBaseTable::Record& record,size_t fieldIndex) const
	...
	switch(f.dataType)
		...
		default:
			Misc::throwStdErr("XBaseTable::getFieldNumber: Mismatching field data type \"%c\"",f.dataType);
		}
...
XBaseTable::Maybe<XBaseTable::Date> XBaseTable::getFieldDate(const XBaseTable::Record& record,size_t fieldIndex) const
	...
	switch(f.dataType)
		...
		default:
			Misc::throwStdErr("XBaseTable::getFieldDate: Mismatching field data type \"%c\"",f.dataType);
		}
