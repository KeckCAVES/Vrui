...
#include <Misc/ASCIIFileReader.h>
...
ASCIIFileReader::ASCIIFileReader(const char* inputFileName)
	...
	if(hasCaseExtension(inputFileName,".gz"))
		{
		/* Open the compressed input file: */
		if((gzInputFile=gzopen(inputFileName,"r"))==0)
			throwStdErr("ASCIIFileReader::ASCIIFileReader: Could not open compressed input file %s",inputFileName);
		}
	else
		{
		/* Open the uncompressed input file: */
		if((inputFd=open(inputFileName,O_RDONLY))<0)
			throwStdErr("ASCIIFileReader::ASCIIFileReader: Could not open input file %s",inputFileName);
		}
...
ASCIIFileReader::ASCIIFileReader(int sInputFd)
	...
	if((inputFd=dup(sInputFd))<0)
		throwStdErr("ASCIIFileReader::ASCIIFileReader: Could not attach to input file descriptor %d",sInputFd);
